...
- Cboe VIX History CSV: https://cdn.cboe.com/api/global/us_indices/daily_prices/VIX_History.csv — `multi-asset-morning-briefing`이 미국 최근 완료 세션과 VIX 종가(`pt`)를 확인하는 공식 read-only 원천.
- ECB Data API EXR: https://data-api.ecb.europa.eu/service/data/EXR/ — `multi-asset-morning-briefing`이 EUR 기준 USD·JPY·GBP reference rate를 받아 EUR/USD·USD/JPY·GBP/USD와 통화쌍별 단위를 계산하는 공식 원천.
- U.S. Treasury daily par yield curve CSV: https://home.treasury.gov/resource-center/data-chart-center/interest-rates — 미국 세션과 관측일이 일치할 때 2Y·10Y·30Y와 2s10s를 계산하는 공식 원천.
