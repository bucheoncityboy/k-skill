...
import { bp, classify, date, days, shift, TENORS } from './core.mjs';
const FIELDS = {
...
const cleanError = (error) => (error instanceof Error ? error.message : String(error)).replace(/[\r\n\t]+/g, ' ').slice(0, 240) || 'unknown error';
export function ustMonthUrl(month) {
    if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(month))
        throw Error('Invalid UST month');
...
function oneTag(xml, name) {
    const matches = [...xml.matchAll(new RegExp(`<d:${name}(?:\\s[^>]*)?>([^<]*)<\\/d:${name}>`, 'g'))];
    const nulls = [...xml.matchAll(new RegExp(`<d:${name}\\s+[^>]*m:null="true"[^>]*/>`, 'g'))];
    if (matches.length + nulls.length !== 1)
        throw Error(`UST XML field ${name} missing or duplicated`);
    if (nulls.length)
        return null;
    return matches[0][1].trim();
}
export function parseUstXml(raw, month) {
    if (raw.length > 5_000_000 || !raw.includes('<feed ') || !raw.includes('<title type="text">DailyTreasuryYieldCurveRateData</title>'))
        throw Error('Invalid UST XML feed');
    const entries = [...raw.matchAll(/<entry>([\s\S]*?)<\/entry>/g)];
    if (!entries.length)
        throw Error(`UST feed has no entries for ${month}`);
    const rows = [];
    const seen = new Set();
    for (const [, entry] of entries) {
        const props = [...entry.matchAll(/<m:properties>([\s\S]*?)<\/m:properties>/g)];
        if (props.length !== 1)
            throw Error('UST XML properties missing or duplicated');
        const timestamp = oneTag(props[0][1], 'NEW_DATE');
        if (!timestamp || !/^\d{4}-\d{2}-\d{2}T00:00:00$/.test(timestamp))
            throw Error('Invalid UST observation date');
        const observationDate = date(timestamp.slice(0, 10));
        if (!observationDate.startsWith(`${month}-`) || seen.has(observationDate))
            throw Error('UST month/date mismatch or duplicate observation');
        ...
        for (const tenor of TENORS) {
            ...
            if (!/^-?\d+(?:\.\d+)?$/.test(value))
                throw Error(`Invalid UST ${tenor}Y yield`);
            const numeric = Number(value);
            if (!Number.isFinite(numeric) || numeric < 0 || numeric > 50)
                throw Error(`UST ${tenor}Y yield outside annual-percent bounds`);
            yields[tenor] = numeric;
        }
        if (complete)
            rows.push({ date: observationDate, yields });
    }
    return rows.sort((a, b) => a.date.localeCompare(b.date));
}
export function ustMonths(base, asOf, maxLag) {
    date(base);
    date(asOf);
    if (base >= asOf || days(asOf, base) > 366)
        throw Error('UST comparison dates must be ordered within 366 days');
    if (!Number.isInteger(maxLag) || maxLag < 0 || maxLag > 14)
        throw Error('UST maxLag must be 0..14');
    const months = new Set();
    for (const anchor of [base, asOf]) {
        for (let d = shift(anchor, -maxLag); d <= anchor; d = shift(d, 1))
            months.add(d.slice(0, 7));
    }
    return [...months].sort();
}
async function fetchUst(url, month, fetcher) {
    let error = Error('UST request failed');
        ...
        try {
            ...
            if (!response.ok) {
                if (response.status >= 400 && response.status < 500 && response.status !== 429)
                    throw Error(`UST HTTP ${response.status} (fatal)`);
                throw Error(`UST HTTP ${response.status}`);
            }
            const raw = await response.text();
            if (raw.length > 5_000_000)
                throw Error('UST response exceeds 5 MB');
            parseUstXml(raw, month);
            return raw;
        }
        catch (e) {
            error = e instanceof Error ? e : Error(String(e));
            if (error.message.endsWith('(fatal)'))
                break;
            if (attempt < 3)
...
async function writeUstCache(file, source) {
...
export async function collectUst(base, asOf, maxLag, cacheDir, useCache = true, fetcher = fetch, now = new Date()) {
    if (!Number.isFinite(now.getTime()))
        throw Error('Invalid UST collection clock');
    const sources = [];
    for (const month of ustMonths(base, asOf, maxLag)) {
        ...
        if (cached && cached.cacheAgeHours <= (month === currentUsMonth ? 6 : 24)) {
            sources.push(cached);
            continue;
        }
        try {
            const raw = await fetchUst(url, month, fetcher);
            const source = { month, url, retrievedAt: now.toISOString(), status: 'live', raw, sha256: digest(raw) };
            sources.push(source);
            if (useCache) {
                try {
                    await mkdir(cacheDir, { recursive: true });
                    await writeUstCache(file, source);
                }
                catch { /* live data remains valid */ }
            }
        }
        catch (e) {
            if (cached)
                ...
                sources.push({ month, url, retrievedAt: now.toISOString(), status: 'failed', raw: '', sha256: '', error: cleanError(e) });
        }
    }
    return { version: 1, engine: UST_ENGINE, base, asOf, maxLag, sources };
}
export function analyzeUst(packet) {
    if (packet.version !== 1 || packet.engine !== UST_ENGINE || !Array.isArray(packet.sources))
        throw Error('Unsupported UST evidence packet');
    const expected = ustMonths(packet.base, packet.asOf, packet.maxLag);
    if (packet.sources.length !== expected.length)
        throw Error('UST source month count mismatch');
    const rows = [];
    let omittedIncompleteDates = 0;
    for (let i = 0; i < expected.length; i++) {
        const source = packet.sources[i];
        if (!source || source.month !== expected[i] || source.url !== ustMonthUrl(source.month) || !['live', 'cache', 'failed'].includes(source.status) || !Number.isFinite(Date.parse(source.retrievedAt)) || new Date(source.retrievedAt).toISOString() !== source.retrievedAt)
            throw Error('UST source identity or checksum mismatch');
        if (source.status === 'failed') {
            if (source.raw !== '' || source.sha256 !== '' || typeof source.error !== 'string' || !source.error || source.error.length > 240 || /[\r\n\t]/.test(source.error) || source.cacheAgeHours !== undefined)
                throw Error('Invalid failed UST source receipt');
            continue;
        }
        if (source.error !== undefined)
            throw Error('Successful UST source cannot contain an error');
        if (source.sha256 !== digest(source.raw))
            throw Error('UST source checksum mismatch');
        if (source.status === 'cache' && (!Number.isFinite(source.cacheAgeHours) || source.cacheAgeHours < 0 || source.cacheAgeHours > 168))
            throw Error('Invalid UST cache age');
        if (source.status === 'live' && source.cacheAgeHours !== undefined)
            throw Error('Live UST source cannot claim cache age');
    ...
    const changes = comparable ? Object.fromEntries(TENORS.map(t => [t, bp(current.yields[t], previous.yields[t])])) : null;
    const spreads = Object.fromEntries([['5Y-3Y', 5, 3], ['10Y-5Y', 10, 5], ['10Y-3Y', 10, 3], ['30Y-10Y', 30, 10]].map(([name, longer, shorter]) => [name, current.yields ? bp(current.yields[longer], current.yields[shorter]) : null]));
    const notes = [current.reason, previous.reason, ...(current.actual === previous.actual && current.actual ? ['두 기준일이 같은 UST 관측일로 정렬되어 변화 계산을 차단했습니다.'] : []), ...(omittedIncompleteDates ? [`UST 만기 결측이 있는 ${omittedIncompleteDates}개 관측일을 제외했습니다.`] : []), ...packet.sources.filter(s => s.status === 'cache').map(s => `${s.month}: 검증된 UST 캐시 사용`), ...packet.sources.filter(s => s.status === 'failed').map(s => `${s.month}: UST 원천 수집 실패 (${s.error})`)].filter((x) => !!x);
