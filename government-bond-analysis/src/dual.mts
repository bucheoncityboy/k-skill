...
import { analyze, bp, TENORS, type Analysis, type Packet } from './core.mjs';
import { analyzeUst, UST_ENGINE, UST_SOURCE, UST_SOURCE_PAGE, type UstAnalysis, type UstPacket, type UstRow } from './ust.mjs';
import { analyzeSchedule, type ScheduleEvidence, type ScheduleAnalysis } from './schedule.mjs';

export const DUAL_ENGINE = 'ktb-ust-rate-analysis/1';
export interface DualPacket { version: 1; engine: typeof DUAL_ENGINE; ktb: Packet; ust: UstPacket; schedule?: ScheduleEvidence }
export interface CrossGap { requested: string; actual: string | null; ust10Y: number | null; ktb10Y: number | null; gapBp: number | null }
export interface DualAnalysis { status: 'READY' | 'INCOMPLETE' | 'BLOCKED'; ktb: Analysis; ust: UstAnalysis; cross: { current: CrossGap; previous: CrossGap; changeBp: number | null }; schedule: ScheduleAnalysis; notes: string[] }
const hash = (text: string): string => createHash('sha256').update(text).digest('hex');
const esc = (s: unknown): string => String(s).replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' })[c]!);
const mdLine = (s: string): string => s.replace(/[\r\n\t]+/g, ' ').replace(/[\\`*_\[\]<>|]/g, '\\$&');
const crossLabel = (a: DualAnalysis): string => a.cross.current.gapBp === null ? '계산 불가' : `${a.cross.current.gapBp.toFixed(2)}bp`;

export function analyzeDual(packet: DualPacket): DualAnalysis {
  if (packet.version !== 1 || packet.engine !== DUAL_ENGINE || !packet.ktb || !packet.ust) throw Error('Unsupported dual-market evidence packet');
  if (packet.ktb.asOf !== packet.ust.asOf || packet.ktb.base !== packet.ust.base || packet.ktb.maxLag !== packet.ust.maxLag) throw Error('KTB/UST request date or lag mismatch');
  const ktb = analyze(packet.ktb), ust = analyzeUst(packet.ust), schedule = analyzeSchedule(packet.schedule, packet.ktb.asOf);
  const common = (requested: string): CrossGap => {
    const krDates = new Map<string, Set<number>>();
    for (const row of packet.ktb.observations) {
      if (!krDates.has(row.date)) krDates.set(row.date, new Set());
    ...
    const us10 = new Map<string, number>(ust.rows.map((r: UstRow) => [r.date, r.yields[10]]));
...
function marketTable(name: string, base: Record<number, number> | null, current: Record<number, number> | null, changes: Record<number, number> | null): string {
...
export function dualSummary(packet: DualPacket, analysis = analyzeDual(packet)): Record<string, unknown> {
  const market = (name: 'KTB' | 'UST', a: Analysis | UstAnalysis) => ({ market: name, status: a.status, observed: { base: a.previous.actual, asOf: a.current.actual }, rates: TENORS.map((tenor, i) => ({ tenor: `${tenor}Y`, base: name === 'KTB' ? (a as Analysis).previous.values?.[i] ?? null : (a as UstAnalysis).previous.yields?.[tenor] ?? null, asOf: name === 'KTB' ? (a as Analysis).current.values?.[i] ?? null : (a as UstAnalysis).current.yields?.[tenor] ?? null, changeBp: name === 'KTB' ? (a as Analysis).changes?.[i] ?? null : (a as UstAnalysis).changes?.[tenor] ?? null })), movement: a.movement, notes: a.notes });
...
const rateText = (value: number | null | undefined): string => value === null || value === undefined ? '—' : `${value.toFixed(3)}%`;
const changeText = (value: number | null | undefined): string => value === null || value === undefined ? '—' : `${value > 0 ? '+' : ''}${value.toFixed(2)}bp`;
const observedText = (requested: string, actual: string | null, lag: number | null): string =>
  actual ? `${actual}${lag ? ` (요청일 ${requested}보다 ${lag}일 전)` : ''}` : '확인 불가';

/** Stable, source-backed Markdown for direct conversational replies. */
export function dualChat(packet: DualPacket, a = analyzeDual(packet)): string {
  ...
  const headline = (name: string, short: number | null, longer: number | null, movement: string): string =>
    short === null || longer === null ? `${name}는 비교 가능한 두 관측일이 부족합니다.` :
      `${name} 3년 ${changeText(short)}, 10년 ${changeText(longer)} (${movement}).`;
  const table = TENORS.map((tenor, i) =>
    `| ${tenor}년 | ${rateText(a.ktb.current.values?.[i])} | ${changeText(a.ktb.changes?.[i])} | ${rateText(a.ust.current.yields?.[tenor])} | ${changeText(a.ust.changes?.[tenor])} |`
  );
...
function dualCsv(a: DualAnalysis): string {
  const cells = ['market,date_role,observation_date,tenor,yield_pct,change_bp'];
  for (const [market, detail] of [['KTB', a.ktb], ['UST', a.ust]] as const) {
    for (const [role, point] of [['base', detail.previous], ['as_of', detail.current]] as const) {
      for (const [i, tenor] of TENORS.entries()) {
        const yieldValue = market === 'KTB' ? (point as Analysis['current']).values?.[i] : (point as UstAnalysis['current']).yields?.[tenor];
...
function dualHtml(packet: DualPacket, a: DualAnalysis): string {
...
const FILES = ['brief.md', 'market-table.csv', 'report.html', 'evidence.mjson', 'status.mjson'] as const;
function scheduleBrief(a: DualAnalysis): string {
...
export async function writeDualArtifacts(out: string, packet: DualPacket): Promise<void> {
  ...
  try {
    ...
    const files: Record<(typeof FILES)[number], string> = {
    ...
    for (const name of FILES) await writeFile(join(stage, name), files[name]);
...
export async function validateDualArtifacts(dir: string): Promise<void> {
  const manifest = JSON.parse(await readFile(join(dir, 'manifest.mjson'), 'utf8')) as { version: number; engine: string; status: string; evidenceSha256: string; files: Record<string, string> };
  if (manifest.version !== 1 || manifest.engine !== DUAL_ENGINE || !manifest.files || JSON.stringify(Object.keys(manifest.files).sort()) !== JSON.stringify([...FILES].sort()) || JSON.stringify((await readdir(dir)).sort()) !== JSON.stringify([...FILES, 'manifest.mjson'].sort())) throw Error('Dual report manifest contract mismatch');
  const evidence = await readFile(join(dir, 'evidence.mjson'), 'utf8');
  if (manifest.evidenceSha256 !== hash(evidence)) throw Error('Dual evidence checksum mismatch');
  const packet = JSON.parse(evidence) as DualPacket, a = analyzeDual(packet);
  if (manifest.status !== a.status) throw Error('Dual status mismatch');
  const expected: Record<(typeof FILES)[number], string> = {
    'brief.md': `# 한·미 국채 금리 분석\n\n요청 ${packet.ktb.base} → ${packet.ktb.asOf} · ${a.status}\n\nKTB 관측 ${a.ktb.previous.actual ?? '없음'} → ${a.ktb.current.actual ?? '없음'} · ${a.ktb.movement}\n\nUST 관측 ${a.ust.previous.actual ?? '없음'} → ${a.ust.current.actual ?? '없음'} · ${a.ust.movement}\n\nUST 10Y − KTB 10Y: ${crossLabel(a)} (공통 관측일 ${a.cross.current.actual ?? '없음'})\n\n${scheduleBrief(a)}\n${a.notes.map(n => `- ${mdLine(n)}`).join('\n')}\n\n원천: https://ecos.bok.or.kr/ · ${UST_SOURCE_PAGE}\n`,
  ...
  for (const name of FILES) {
    ...
    if (hash(content) !== manifest.files[name] || content !== expected[name]) throw Error(`${name}: hash or deterministic output mismatch`);
  }
  if (a.status === 'READY' && (!expected['report.html'].includes('원천·수집 기록') || !expected['report.html'].includes(UST_SOURCE_PAGE))) throw Error('Dual READY report lacks source provenance');
}
