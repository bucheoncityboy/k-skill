...
import { analyze, TENORS, type Analysis, type Packet } from './core.mjs';
import { analyzeNews, type NewsAnalysis } from './news.mjs';

const h=(s:string):string=>createHash('sha256').update(s).digest('hex');
const esc=(s:unknown):string=>String(s).replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]!));
const md=(s:unknown):string=>String(s).replace(/[\\`*_[\]<>]/g,'\\$&');
const n=(v:number|null,d=2):string=>v===null?'—':v.toFixed(d);
const signed=(v:number|null):string=>v===null?'—':`${v>0?'+':''}${v.toFixed(2)}`;
const trend=(v:number|null):string=>v===null?'판정 불가':v>0?'상승':v<0?'하락':'보합';
const spreadTrend=(v:number|null):string=>v===null?'판정 불가':v>0?'확대':v<0?'축소':'보합';

export function marketCsv(p:Packet,a:Analysis):string{
...
export function brief(p:Packet,a:Analysis,news:NewsAnalysis=analyzeNews(p.newsEvidence,a)):string{
  ...
  if(a.changes&&a.current.values&&a.previous.values){
    ...
    lines.push('## 핵심 변화','',
      `- 국고채 3년은 ${signed(a.changes[i3])}bp ${trend(a.changes[i3])}, 5년은 ${signed(a.changes[i5])}bp ${trend(a.changes[i5])}, 10년은 ${signed(a.changes[i10])}bp ${trend(a.changes[i10])}.`,
      `- 10년−3년 스프레드는 ${n(s103.current)}bp이며 비교일보다 ${n(Math.abs(s103.change??0))}bp ${spreadTrend(s103.change)}.`,
      `- 곡선 움직임: **${a.movement}**.`,'');
  } else lines.push('## 결과 제한','', '- 비교 가능한 여섯 만기의 공통 관측값이 없어 변화 계산과 문장 생성을 차단했습니다.','');
  if(news.status!=='DISABLED')lines.push('## 시장 배경','',`- 뉴스 근거 상태: **${news.status}**${news.confidence?` / 신뢰도 **${news.confidence}**`:''}`,...news.commentary.map(x=>`- ${md(x)}`),...news.notes.map(x=>`- 주의: ${md(x)}`),...([...news.eligible.map(x=>({label:'채택',article:x})),...news.context.map(x=>({label:'맥락',article:x})),...news.excluded.map(x=>({label:'제외',article:x}))].map(({label,article:x})=>`- ${label} · ${md(x.publisher)} · ${md(x.title)} · ${x.publishedAt} · [원문](${x.url}) · 근거 SHA-256 ${x.sourceFingerprint}`)),'','- 기사에서 전한 시장 해석을 출처와 함께 요약했으며, ECOS 수치만으로 인과관계를 확정하지 않는다.','');
  lines.push('## 검증 메모','',...(a.notes.length?a.notes.map(x=>`- ${md(x)}`):['- 결측·날짜 정렬 경고 없음.']),'',
    '## 출처','',...p.receipts.map(r=>`- ${r.tenor}년: ${md(r.sourceId)} / ${md(r.statCode)}·${md(r.itemCode)} ${md(r.itemName)} / ${md(r.unit)}·${md(r.cycle)} / ${r.first??'없음'}~${r.last??'없음'} (${r.count}건) / ${r.status} / 원문 SHA-256 ${r.rawSha256.join(', ')||'없음'}${r.sourceUrl?` / ${r.sourceUrl}`:''}`),'',
    '## 해석 범위','', '- 이 결과는 공식 통계의 기간 비교다. 실시간 호가, 원인 추정, 금리 전망 또는 투자 권유가 아니다.','');
  return lines.join('\n')+'\n';
}

function chart(a:Analysis):string{
  if(!a.current.values||!a.previous.values)return '<div class="empty">공통 관측값이 없어 차트를 생성하지 않았습니다.</div>';
  const all=[...a.current.values,...a.previous.values],min=Math.min(...all)-.03,max=Math.max(...all)+.03,range=Math.max(max-min,.1);
  const x=(i:number)=>75+i*(500/(TENORS.length-1)), y=(v:number)=>260-(v-min)/range*190;
  const path=(values:number[])=>values.map((v,i)=>`${i?'L':'M'} ${x(i)} ${y(v).toFixed(1)}`).join(' ');
  ...
  const dots=(values:number[],cls:string)=>values.map((v,i)=>`<circle class="${cls}" cx="${x(i)}" cy="${y(v)}" r="5"/>`).join('');
  const labels=(values:number[],other:number[],cls:string)=>values.map((v,i)=>{const above=y(v)<=y(other[i]),baseline=y(v)+(above?-14:22),top=baseline-13;return `<g class="value-label ${cls}"><rect x="${x(i)-24}" y="${top}" width="48" height="18" rx="5"/><text x="${x(i)}" y="${baseline}" text-anchor="middle">${v.toFixed(3)}</text></g>`;}).join('');
...
export function html(p:Packet,a:Analysis,news:NewsAnalysis=analyzeNews(p.newsEvidence,a)):string{
...
const artifactNames=['market-table.csv','brief.md','report.html','evidence.mjson','status.mjson'] as const;
function statusJson(p:Packet,a:Analysis,news:NewsAnalysis):string{return JSON.stringify({status:a.status,notes:a.notes,usable:a.status==='READY',news:{status:news.status,confidence:news.confidence,curveDirection:news.curveDirection,notes:news.notes,sources:[...news.eligible.map(x=>({disposition:'eligible',...x})),...news.context.map(x=>({disposition:'context',...x})),...news.excluded.map(x=>({disposition:'excluded',...x}))].map(({disposition,publisher,title,url,publishedAt,sourceFingerprint})=>({disposition,publisher,title,url,publishedAt,sourceFingerprint}))},sources:p.receipts.map(({tenor,status,sourceId,statCode,itemCode,unit,cycle,count,first,last,retrievedAt,rawSha256})=>({tenor,status,sourceId,statCode,itemCode,unit,cycle,count,first,last,retrievedAt,rawSha256}))},null,2)+'\n';}
export interface Manifest { version:5; engine:string; status:Analysis['status']; newsStatus:NewsAnalysis['status']; evidenceSha256:string; files:Record<string,string> }
export async function writeArtifacts(out:string,p:Packet):Promise<Manifest>{
  ...
  try{await stat(out);throw Error(`Output path already exists: ${out}`);}catch(e){if((e as NodeJS.ErrnoException).code!=='ENOENT')throw e;}
  try{
    await mkdir(stage,{recursive:false});
    const evidence=JSON.stringify(p,null,2)+'\n';
    const files:Record<string,string>={'market-table.csv':marketCsv(p,a),'brief.md':brief(p,a,news),'report.html':html(p,a,news),'evidence.mjson':evidence,'status.mjson':statusJson(p,a,news)};
    const manifest:Manifest={version:5,engine:p.engine,status:a.status,newsStatus:news.status,evidenceSha256:h(evidence),files:{}};
    for(const name of artifactNames){const content=files[name];if(/NaN|undefined/.test(content))throw Error(`${name}: invalid token`);manifest.files[name]=h(content);await writeFile(join(stage,name),content);}
    await writeFile(join(stage,'manifest.mjson'),JSON.stringify(manifest,null,2)+'\n');await validateArtifacts(stage);await rename(stage,out);return manifest;
  }catch(e){await rm(stage,{recursive:true,force:true});throw e;}
}
export async function validateArtifacts(dir:string):Promise<void>{
  const manifest=JSON.parse(await readFile(join(dir,'manifest.mjson'),'utf8')) as Manifest;
  if(manifest.version!==5||manifest.engine!=='korean-interest-rate-analysis/5'||!manifest.files||typeof manifest.files!=='object')throw Error('Unsupported manifest');
  const expected=[...artifactNames].sort(),declared=Object.keys(manifest.files).sort(),present=(await readdir(dir)).sort();
  if(JSON.stringify(declared)!==JSON.stringify(expected)||JSON.stringify(present)!==JSON.stringify([...expected,'manifest.mjson'].sort())||Object.values(manifest.files).some(x=>typeof x!=='string'||!/^[a-f0-9]{64}$/.test(x))||!/^[a-f0-9]{64}$/.test(manifest.evidenceSha256))throw Error('Manifest file contract mismatch');
  const evidence=await readFile(join(dir,'evidence.mjson'),'utf8');
  if(h(evidence)!==manifest.evidenceSha256)throw Error('Evidence hash mismatch');
  const p=JSON.parse(evidence) as Packet,a=analyze(p),news=analyzeNews(p.newsEvidence,a);
  if(a.status!==manifest.status||news.status!==manifest.newsStatus)throw Error('Status mismatch');
  for(const [name,digest] of Object.entries(manifest.files)){const content=await readFile(join(dir,name),'utf8');if(h(content)!==digest)throw Error(`${name}: hash mismatch`);}
  const mdFile=await readFile(join(dir,'brief.md'),'utf8'),csv=await readFile(join(dir,'market-table.csv'),'utf8'),page=await readFile(join(dir,'report.html'),'utf8'),status=await readFile(join(dir,'status.mjson'),'utf8');
  if(mdFile!==brief(p,a,news)||csv!==marketCsv(p,a)||page!==html(p,a,news)||status!==statusJson(p,a,news))throw Error('Rendered artifact is not reproducible');
  if(a.status==='READY'&&(!page.includes('<svg')||csv.trim().split(/\r?\n/).length!==1+TENORS.length+a.spreads.length||!mdFile.includes('## 핵심 변화')||!page.includes('data-provenance="true"')||!page.includes('class="value-label')||page.includes('원문 SHA-256')||p.receipts.some(r=>r.sourceUrl&&!page.includes(r.sourceUrl))))throw Error('READY output contract is incomplete');
  if(a.status!=='READY'&&!mdFile.includes('## 결과 제한'))throw Error('Non-ready output must state its limitation');
}
