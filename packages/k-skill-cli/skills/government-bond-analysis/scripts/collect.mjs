...
import { SOURCE_ID, SOURCE_URL, TENORS, TENOR_META, date, days, normalize, parseEcos, sha256, shift } from './core.mjs';
...
export async function request(url, get = fetch, pause = ms => new Promise(r => setTimeout(r, ms)), validate) {
...
const cacheName = (tenor, from, to) => `${tenor}-${from}-${to}.mjson`;
    ...
    const parts = await Promise.all(TENORS.map(async (tenor) => {
        ...
        for (const segment of segments) {
            ...
            try {
                ...
                const result = await request(url, get, undefined, text => { parse(text, tenor, segment.from, segment.to); });
