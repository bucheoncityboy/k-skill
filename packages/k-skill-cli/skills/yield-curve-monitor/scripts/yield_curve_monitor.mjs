...
function args(argv) {
    const out = {}, allowed = new Set(['as-of', 'base', 'from-month', 'to-month', 'preset', 'event-date', 'lookback-sessions', 'out', 'input', 'replay', 'news-evidence', 'news-template', 'cache-dir', 'max-lag', 'no-cache', 'text', 'help']);
    for (let i = 0; i < argv.length; i++) {
        ...
        if (['no-cache', 'text', 'help'].includes(k))
            out[k] = true;
        else {
            const v = argv[++i];
            if (!v || v.startsWith('--'))
...
const command = "npx -y @nomadamas/k-skill@0 exec yield-curve-monitor scripts/yield_curve_monitor.mjs --";
const usage = `Usage:
  ${command} [--as-of YYYY-MM-DD] [--base YYYY-MM-DD] [--max-lag 0..14] [--text]
  ${command} --from-month YYYY-MM --to-month YYYY-MM [--text]
  ${command} --preset today|dashboard|previous-session|previous-week|previous-month-end|previous-quarter-end|year-start|policy-gap [--as-of YYYY-MM-DD] [--text]
  ${command} --event-date YYYY-MM-DD [--text]
  ${command} --lookback-sessions 5|20 [--as-of YYYY-MM-DD] [--text]
  ${command} --input FILE.csv --as-of YYYY-MM-DD --base YYYY-MM-DD [--text]
  ${command} --base YYYY-MM-DD --as-of YYYY-MM-DD --news-evidence FILE.json [--out DIR]
  ${command} --news-template YYYY-MM-DD
  ${command} --replay evidence.json [--text]
...
async function main() {
    ...
    if (hasReplay && Object.keys(a).some(k => !['replay', 'out', 'text'].includes(k)))
        throw Error('--replay accepts only --out and --text');
    ...
    const cacheDir = resolve(typeof a['cache-dir'] === 'string' ? a['cache-dir'] : join(homedir(), '.cache', 'k-skill', 'yield-curve-monitor')), useCache = a['no-cache'] !== true;
    ...
    if (typeof a.replay === 'string') {
        ...
        try {
            const m = JSON.parse(await readFile(join(dirname(evidencePath), 'manifest.json'), 'utf8'));
        ...
        p = { version: PACKET_VERSION, engine: ENGINE, asOf, base, maxLag, observations, receipts, label: '국고채 수익률곡선 검증 보고서' };
    }
    else {
        if (asOf > kstToday())
            throw Error('Live collection cannot use a future KST as-of date');
        const result = await collect(base, asOf, maxLag, cacheDir, useCache);
        p = { version: PACKET_VERSION, engine: ENGINE, asOf, base, maxLag, ...result, label: '국고채 수익률곡선 검증 보고서' };
    }
    ...
    if (typeof a.out === 'string') {
        ...
        console.log(JSON.stringify({ result: manifest.status, out, files: [...Object.keys(manifest.files), 'manifest.json'] }, null, 2));
    }
    else
