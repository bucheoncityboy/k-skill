# yield-curve-monitor — assembled instructions
...
- Execute bundled helpers only through `npx -y @nomadamas/k-skill@0 exec yield-curve-monitor scripts/<file> -- <args>`; do not assume a repository-relative or installed-skill-relative path.
- Resolve an asset path with `npx -y @nomadamas/k-skill@0 path yield-curve-monitor <relative-path>` only when another tool explicitly requires a filesystem path.
- Read bundled references through `npx -y @nomadamas/k-skill@0 read yield-curve-monitor references/<file>`.
