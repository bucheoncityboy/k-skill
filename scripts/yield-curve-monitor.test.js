...
const skillDir = path.join(repoRoot, "yield-curve-monitor");
const cli = path.join(skillDir, "scripts", "yield_curve_monitor.mjs");
const artifactNames = [
  "brief.md",
  "evidence.json",
  "manifest.json",
...
test("yield-curve helper exposes the documented CLI", async () => {
  const { stdout } = await run(process.execPath, [cli, "--help"], { cwd: skillDir });
  assert.match(stdout, /@nomadamas\/k-skill@0 exec yield-curve-monitor scripts\/yield_curve_monitor\.mjs --/);
...
test("report bundle is byte-identical when replayed", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "yield-curve-monitor-"));
